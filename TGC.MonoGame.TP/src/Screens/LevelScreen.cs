...
namespace TGC.Monogame.TP.Src.Screens 
{
    ...
    {        
        ...
        public override void Draw()
        {
            ...
            var frontCarPosition = new Vector3(Car.Position.X, Car.Position.Y, Car.Position.Z) - new Vector3(forward.X, 0, forward.Z) * 2000;
            ...
            Lights.Parameters["frontCarPosition"]?.SetValue(frontCarPosition);
            Lights.Parameters["KAmbient"]?.SetValue(0.5f);
            Lights.Parameters["KSpecular"]?.SetValue(0.5f);
            Lights.Parameters["Alcance"]?.SetValue(300.0f);
            Lights.Parameters["VarAngulo"]?.SetValue(2.2f);//mayor = menos angulo
            ...
            for (int i = 0; i < Mounts.Length; i++) Mounts[i].Draw(View, Projection, Lights);
            for (int i = 0; i < BoostPads.Length; i++) BoostPads[i].Draw(View, Projection);
            for (int i = 0; i < Trees.Length; i++) Trees[i].Draw(View, Projection, Lights);
            Lights.CurrentTechnique = Lights.Techniques["Box"];
